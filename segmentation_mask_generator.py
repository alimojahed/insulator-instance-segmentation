...
def get_segmentation_mask(bbox, image):
    # print(image)
...
def augment(labels):
    ...
    for index in tqdm(range(len(img_ids))):
        ...
        # if not img_path == "augmented_images/1367.jpg":
        #     continue
        for ann in coco_file.imgToAnns[img_id]:
            mask = get_segmentation_mask(ann["bbox"], image)
        ...
        for i in range(balancing_factor-1):
            ...
            cv2.imwrite("./augmented_images/" + new_image_path, augmented['image'])

            for index, ann in enumerate(augmented['bboxes']):
                mask = get_segmentation_mask(ann, augmented['image'])
                segmentations = create_sub_mask_annotation(mask)
                json_coco["annotations"].append({
                    "segmentation": [],
                    "area": ann[2] * ann[3],
